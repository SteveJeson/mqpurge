...
namespace ChatServer.Codec
{
    public class MsgDecoder
    {
        ...
        public JT808_PackageData.LocationInfo ToLocationInfoMsg(byte[] data)
        {
            ...
            //基本信息
            locationInfo.deviceCode = data[0].ToString("X2") + data[1].ToString("X2") +
                                data[2].ToString("X2") + data[3].ToString("X2") +
                                data[4].ToString("X2") + data[5].ToString("X2");//终端设备号 消息体前6个byte
            ...
            locationInfo.lat = (double)ExplainUtils.ParseIntFromBytes(data, 14, 4) / 1000000; //纬度 4位 除以10的6次方 转为度
            locationInfo.lon = (double)ExplainUtils.ParseIntFromBytes(data, 18, 4) / 1000000;//经度 4位 除以10的6次方 转为度
            ...
            {
                logger.Info("error happens when parsing time! "+e.Message);
            }
